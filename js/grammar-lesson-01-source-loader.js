(function () {
  ...
  function extractRules(source) {
    var rules = [];
    var re = /title:\s*'([^']+)'\s*,\s*html:\s*`([\s\S]*?)`/g;
    var match;
    while ((match = re.exec(source))) {
      rules.push({ title: match[1], html: match[2] });
    }
    return rules;
  }

  function render(rules) {
    var lesson = document.querySelector('.grammar-lesson');
    if (!lesson) return false;

    var content = lesson.querySelector('.grammar-content');
    if (!content) return false;
    if (content.dataset.lesson01SourceLoaded === 'true') return true;
    ...
    rules.forEach(function (rule, index) {
      ...
      item.id = 'lesson01-source-rule-' + (index + 1);
    ...
    content.dataset.lesson01SourceLoaded = 'true';
    return true;
  }
  ...
  function init() {
    fetch(SOURCE_URL, { cache: 'no-store' })
      .then(function (response) {
        if (!response.ok) throw new Error('Could not load Lesson 01 source text');
        return response.text();
      })
      .then(function (source) {
        var rules = extractRules(source);
        if (!rules.length) throw new Error('No Lesson 01 rules found');

        var attempts = 0;
        var timer = setInterval(function () {
          attempts++;
          if (render(rules) || attempts > 100) clearInterval(timer);
        }, 100);
      })
      .catch(function (error) {
        console.error('[Grammar Lesson 01]', error);
      });
  }
