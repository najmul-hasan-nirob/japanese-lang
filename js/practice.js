...
(() => {
    ...
    function renderLessonChoices(){
        ...
        // Empty selection is intentional on first visit. Do not auto-select all lessons.
    ...
    function start(){if(!selectedLessons.length){updateStats();return;}if(practiceMode==="hard")queue=hardItems().slice().sort(()=>Math.random()-.5);else{queue=dueItems().slice().sort(()=>Math.random()-.5);if(!queue.length)queue=newItems().slice(0,10).sort(()=>Math.random()-.5);}index=0;answered=0;document.getElementById("practiceStart").hidden=true;document.getElementById("practiceDone").hidden=true;document.getElementById("practiceCard").hidden=false;if(!queue.length){finish();return;}renderCard();}
    ...
    document.addEventListener("DOMContentLoaded",()=>{if(!document.getElementById("practiceCard"))return;renderLessonChoices();updateStats();document.getElementById("startPractice").addEventListener("click",()=>{practiceMode="due";start();});document.getElementById("hardPractice").addEventListener("click",()=>{practiceMode="hard";updateStats();start();});document.getElementById("restartPractice").addEventListener("click",start);document.getElementById("showAnswer").addEventListener("click",showAnswer);document.getElementById("practiceSpeak").addEventListener("click",()=>{if(queue[index])speak(queue[index]);});document.querySelectorAll("#ratingButtons button").forEach(btn=>btn.addEventListener("click",()=>rate(btn.dataset.rating)));document.getElementById("selectAllLessons").addEventListener("click",()=>{selectedLessons=allLessonKeys();saveLessonSelection();renderLessonChoices();updateStats();});document.getElementById("clearLessons").addEventListener("click",()=>{selectedLessons=[];saveLessonSelection();renderLessonChoices();updateStats();});});
})();
