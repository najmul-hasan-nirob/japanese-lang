...
(() => {
    ...
    function allVocabulary(){
        ...
        sortedLessonKeys().forEach(key=>{
            const lesson=lessonsData[key]; if(!lesson)return;
    ...
    function dueItems(){const t=now();return allVocabulary().filter(item=>{const r=state[item.id];return !r||!r.nextReview||r.nextReview<=t;});}
    ...
    function renderCard(){
        const item=queue[index]; if(!item)return finish();
        ...
        document.getElementById("practiceRomaji").textContent=item.romaji||toRomaji(item.jp);
    ...
    function speak(item){
        if(typeof window.speakJapanese==="function")window.speakJapanese(item.jp);
        else if("speechSynthesis"in window){speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(item.jp);u.lang="ja-JP";speechSynthesis.speak(u);}
    }
    ...
    function rate(kind){
        const item=queue[index], r=getRecord(item), old=r.interval||0;
        ...
        r.nextReview=now()+r.interval;saveState();answered++;index++;renderCard();
    }
    function start(){
        const due=dueItems();queue=due.slice().sort(()=>Math.random()-.5);
        if(!queue.length)queue=newItems().slice(0,10).sort(()=>Math.random()-.5);
        index=0;answered=0;document.getElementById("practiceStart").hidden=true;document.getElementById("practiceDone").hidden=true;document.getElementById("practiceCard").hidden=false;renderCard();
    }
