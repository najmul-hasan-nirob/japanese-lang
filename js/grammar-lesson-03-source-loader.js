(function () {
  ...
  function render() {
    ...
    if (!lesson || !window.grammarLesson03Source) return false;

    var content = lesson.querySelector('.grammar-content');
    if (!content) return false;
    if (content.dataset.lesson03SourceLoaded === 'true') return true;

    content.innerHTML = '';

    window.grammarLesson03Source.forEach(function (rule, index) {
      var item = document.createElement('div');
      item.className = 'gs-source-accordion-item';
      item.id = 'lesson03-source-rule-' + (index + 1);
    ...
    content.dataset.lesson03SourceLoaded = 'true';
    return true;
  }
