...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderLessons(){
        ...
        cards.forEach(item=>{
            ...
            if(speakText&&isSpeakableJapanese(speakText))card.appendChild(createSpeakerButton(speakText));
    ...
    allCheckbox.addEventListener("change",()=>{lessonCheckboxes().forEach(cb=>cb.checked=allCheckbox.checked);updateLessonLabel();renderLessons();});
    lessonCheckboxes().forEach(cb=>cb.addEventListener("change",()=>{const boxes=lessonCheckboxes();allCheckbox.checked=boxes.length>0&&boxes.every(box=>box.checked);updateLessonLabel();renderLessons();}));
