...
(() => {
    ...
    const KANJI_SHUFFLE_KEY = "japanese-lang-kanji-shuffle-v1";
    const DEFAULT_LESSON_FILTER = { selectedLessons: ["lesson1"], selectedTypes: ["vocabulary"], orderMode: "normal" };
    ...
    function normalizeLessonFilter(value) {
        ...
        const selectedTypes = Array.isArray(value.selectedTypes) ? value.selectedTypes.filter(item => typeof item === "string") : [];
        return {
            ...
            ...(value.kanjiShuffleState && typeof value.kanjiShuffleState === "object" ? { kanjiShuffleState: value.kanjiShuffleState } : {})
        };
    }
    function normalizeSimilarWordsFilter(value) {
        ...
        return { selectedGroups: selectedGroups.length ? selectedGroups : [...DEFAULT_SIMILAR_WORDS_FILTER.selectedGroups], orderMode: value.orderMode === "shuffle" ? "shuffle" : "normal" };
    }
    function normalizeShuffleStore(value) {
        return value && typeof value === "object" && value.states && typeof value.states === "object" ? value : { version: 1, states: {} };
    }
    function mergeShuffleStores(localValue, cloudValue) {
        const local = normalizeShuffleStore(localValue), cloud = normalizeShuffleStore(cloudValue), states = { ...cloud.states };
        Object.entries(local.states).forEach(([key, localState]) => {
            const cloudState = states[key];
            if (!cloudState || Number(localState?.updatedAt || 0) >= Number(cloudState?.updatedAt || 0)) states[key] = localState;
        });
    ...
    function buildLessonFilterSnapshot() {
        ...
        return {
            ...
            kanjiShuffleState: get(KANJI_SHUFFLE_KEY, null),
            similarWordsFilter: normalizeSimilarWordsFilter(get(SIMILAR_WORDS_FILTER_KEY, DEFAULT_SIMILAR_WORDS_FILTER))
        };
    }
    function snapshot() { return JSON.stringify({ hard: get(HARD_KEY, []), sr: get(SR_KEY, {}), lessons: get(PRACTICE_KEY, []), lessonFilter: buildLessonFilterSnapshot() }); }

    async function pullCloud(userId) {
        if (!client || !userId) return;
        const { data, error } = await client.from(TABLE).select("hard_vocabulary,spaced_repetition,practice_lessons,lesson_filter,updated_at").eq("user_id", userId).maybeSingle();
        ...
        if (data.lesson_filter && typeof data.lesson_filter === "object") {
            ...
            const mergedKanjiShuffle = mergeShuffleStores(get(KANJI_SHUFFLE_KEY, null), cloudFilter.kanjiShuffleState);
            const cloudSimilarWords = normalizeSimilarWordsFilter(data.lesson_filter.similarWordsFilter);
            set(LESSON_FILTER_KEY, { selectedLessons: cloudFilter.selectedLessons, selectedTypes: cloudFilter.selectedTypes, orderMode: cloudFilter.orderMode, shuffleState: mergedShuffle, kanjiShuffleState: mergedKanjiShuffle });
            set(SHUFFLE_KEY, mergedShuffle);
            set(KANJI_SHUFFLE_KEY, mergedKanjiShuffle);
    ...
    async function pushCloud(userId, force = false) {
        ...
        const { error } = await client.from(TABLE).upsert({ user_id: userId, hard_vocabulary: payload.hard, spaced_repetition: payload.sr, practice_lessons: payload.lessons, lesson_filter: payload.lessonFilter, updated_at: new Date().toISOString() }, { onConflict: "user_id" });
        if (error) { console.warn("Japanese Lang cloud sync:", error.message); return; }
        lastSnapshot = snap;
    ...
    function queueCloudPush() {
        ...
        pushTimer = setTimeout(() => pushCloud(currentUserId), 500);
    }
    function stopCloudSync() { clearTimeout(pushTimer); pushTimer = null; currentUserId = null; }
    function createAuthUI() {
        ...
        const style = document.createElement("style"); style.textContent = `#jlCloudAuth{position:fixed;left:14px;bottom:14px;z-index:99999;font-family:inherit}#jlCloudButton{width:42px;height:42px;border:1px solid var(--paper-line,#d9d2c3);border-radius:50%;background:var(--paper-cell,#fffdf8);color:var(--ink,#241f18);cursor:pointer;font-size:19px;box-shadow:var(--shadow,0 2px 10px rgba(0,0,0,.12))}#jlCloudPanel{position:absolute;left:0;bottom:52px;width:250px;padding:14px;border-radius:14px;background:var(--paper-cell,#fffdf8);color:var(--ink,#241f18);box-shadow:0 10px 35px rgba(0,0,0,.2);border:1px solid var(--paper-line,#d9d2c3);display:flex;flex-direction:column;gap:8px}#jlCloudPanel[hidden]{display:none}#jlCloudPanel input,#jlCloudPanel button:not(#jlCloudButton){box-sizing:border-box;width:100%;padding:8px;border-radius:8px;border:1px solid var(--paper-line,#d9d2c3);background:var(--paper,#f7f2e7);color:var(--ink,#241f18);font:inherit}#jlCloudPanel input{font-size:16px}#jlCloudPanel input::placeholder{color:var(--ink-soft,#7a705e)}#jlCloudStatus,#jlCloudMessage{font-size:12px;color:var(--ink-soft,#7a705e)}@media (max-width:520px){#jlCloudAuth{position:static;left:auto;bottom:auto;width:auto;z-index:auto;grid-column:auto;margin:0}#jlCloudButton{width:100%;height:42px;border-radius:6px;box-shadow:none;font-size:16px;min-width:0}#jlCloudPanel{position:absolute;left:12px;right:12px;bottom:auto;top:calc(100% + 6px);width:auto;z-index:10001;max-height:calc(100dvh - 110px);overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;padding-bottom:calc(14px + env(safe-area-inset-bottom));scroll-padding-bottom:24px}#jlCloudPanel input,#jlCloudPanel button:not(#jlCloudButton){min-height:42px}#jlCloudAuth.jl-mobile-open #jlCloudPanel{display:flex}}`; document.head.appendChild(style);
        ...
        document.getElementById("jlSignIn").onclick = () => auth(false); document.getElementById("jlSignUp").onclick = () => auth(true); document.getElementById("jlSignOut").onclick = async () => { await client.auth.signOut(); };
        ...
        placeAuth(); window.addEventListener("resize", placeAuth); document.addEventListener("click", e => { if (window.innerWidth <= 520 && !wrap.contains(e.target)) { wrap.classList.remove("jl-mobile-open"); panel.hidden = true; } });
    }
    ...
    async function init() {
        ...
        if (data.session?.user) { currentUserId = data.session.user.id; await pullCloud(currentUserId); }
        client.auth.onAuthStateChange(async (_event, session) => { updateUI(session); if (session?.user) { currentUserId = session.user.id; await pullCloud(currentUserId); } else stopCloudSync(); });
        window.addEventListener("visibilitychange", () => { if (document.visibilityState === "visible" && currentUserId) pullCloud(currentUserId); });
        window.addEventListener("online", () => { if (currentUserId) pullCloud(currentUserId); });
        window.addEventListener("similarWordsFilterStateChanged", queueCloudPush);
        window.addEventListener("lessonFilterStateChanged", queueCloudPush);
        window.addEventListener("japaneseLangKanjiShuffleChanged", queueCloudPush);
