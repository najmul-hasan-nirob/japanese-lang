(function () {
    ...
    function kanaToRomaji(text) {
        ...
        for (let i = 0; i < text.length; i++) {
            if (text[i] === 'っ') {
                const n = table[text.slice(i + 1, i + 3)] || table[text[i + 1]] || '';
                if (n) out += n.charAt(0);
                continue;
            }
    ...
    function loadDetail(kanji) {
        if (detailCache.has(kanji)) return detailCache.get(kanji);
        const promise = fetch('https://kanjiapi.dev/v1/kanji/' + encodeURIComponent(kanji))
            .then(function (r) { if (!r.ok) throw new Error('Kanji API request failed'); return r.json(); })
            .catch(function () { return {}; });
        detailCache.set(kanji, promise);
        return promise;
    }

    function getDetail(kanji) {
        const value = detailCache.get(kanji);
    ...
    function filteredCards() {
        ...
        return cards.filter(function (item) {
            ...
            if (!q) return true;
            const d = getDetail(item.kanji) || {};
            const text = [item.kanji, item.reading, item.meaning, item.example, item.exampleReading, ...(d.kun_readings || []), ...(d.on_readings || []), ...(d.meanings || [])].concat([kanaToRomaji(item.reading), kanaToRomaji(item.exampleReading)]).join(' ').toLowerCase();
    ...
    function cleanExampleWord(word, target) {
        const original = String(word || '');
        let result = '';
        for (const ch of original) {
            if (ch === target) result += ch;
            else if (/[一-龯々]/.test(ch)) continue;
            else result += ch;
        }
    ...
    function detailHtml(item) {
        ...
        const meaning = item.meaning || d.bangla_meaning || (d.meanings || [])[0] || '';
        const jap = kun.length ? '<div class="kanji-reading-line"><span class="kanji-reading-label">Jap:</span> ' + readingsHtml(kun.join(' ')) + ' - ' + escapeHtml(meaning) + '</div>' : '<div class="kanji-reading-line"><span class="kanji-reading-label">Jap:</span> - ' + escapeHtml(meaning) + '</div>';
        const chi = on.length ? '<div class="kanji-reading-line kanji-chi-line"><span class="kanji-reading-label">Chi:</span> ' + readingsHtml(on.join(' ')) + '</div>' : '';

        let exampleHtml = '';
        if (on.length && item.example && item.exampleReading) {
            const word = cleanExampleWord(item.example, item.kanji);
            const reading = item.exampleReading;
            exampleHtml = '<div class="kanji-reading-line kanji-example-line"><span class="kanji-reading-label">Ex:</span> ' + escapeHtml(word) + ' - ' + escapeHtml(reading) + ' (' + escapeHtml(kanaToRomaji(reading)) + ')</div>';
        }
        return '<div class="kanji-detail-meaning">' + escapeHtml(meaning) + '</div>' + jap + chi + exampleHtml;
    }
    ...
    function render() {
        ...
        grid.querySelectorAll('.card').forEach(function (card) {
            card.addEventListener('click', async function (event) {
                ...
                const item = cards.find(function (x) { return x.kanji === card.dataset.kanji; });
                if (item && !getDetail(item.kanji)) {
                    await loadDetail(item.kanji);
                    const back = card.querySelector('.kanji-details');
                    if (back) back.innerHTML = detailHtml(item);
                }
            });
        });
    ...
    function readUIState() {
        try { const all = JSON.parse(localStorage.getItem(UI_STATE_KEY) || '{}'); return all && typeof all === 'object' ? all : {}; } catch (_) { return {}; }
    }
    function saveUIState() {
        try {
            ...
            levelPanel.querySelectorAll('input[type="checkbox"]').forEach(function (el) { if (el.id) controls[el.id] = { type:'checkbox', checked:el.checked }; });
