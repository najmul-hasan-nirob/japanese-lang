(function () {
  ...
  function render() {
    ...
    if (!lesson || !window.grammarLesson02Source) return false;

    var content = lesson.querySelector('.grammar-content');
    if (!content) return false;
    if (content.dataset.lesson02SourceLoaded === 'true') return true;

    content.innerHTML = '';

    window.grammarLesson02Source.forEach(function (rule, index) {
      var item = document.createElement('div');
      item.className = 'gs-source-accordion-item';
      item.id = 'lesson02-source-rule-' + (index + 1);
    ...
    content.dataset.lesson02SourceLoaded = 'true';
    return true;
  }
