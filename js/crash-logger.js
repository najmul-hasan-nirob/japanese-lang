...
(() => {
    "use strict";
    const KEY="japanese-lang-crash-log-v2", MAX_EVENTS=120, MAX_ERRORS=40;
    ...
    function event(type,details={}){const e={at:now(),type,...details};state.lastSeenAt=e.at;state.lastEvent=e;state.events.push(e);if(state.events.length>MAX_EVENTS)state.events.splice(0,state.events.length-MAX_EVENTS);save()}
    ...
    async function upload(p){try{const r=await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}`,{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(p),keepalive:true});return r.ok}catch(_){return false}}
    async function uploadPrevious(){if(!previous)return false;return upload({session_id:previous.sessionId,detected_at:previous.detectedAt,started_at:previous.startedAt,last_seen_at:previous.lastSeenAt,possible_crash:true,context:previous.context,events:previous.events,errors:previous.errors,last_event:previous.lastEvent,app_state:previous.detectedAppState})}
    event("session-start",{previousSessionDetected:!!previous,previousSessionId:previous?.sessionId||null});
    save();
    if(previous){setTimeout(async()=>{if(await uploadPrevious()){try{localStorage.removeItem(KEY)}catch(_){} }},2000)}
    window.JapaneseLangCrashLogger=Object.freeze({getState:()=>JSON.parse(JSON.stringify(state)),addEvent:(t,d)=>event(String(t),d||{}),addError:(t,d)=>error(String(t),d||{})});
})();
