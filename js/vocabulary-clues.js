...
async function save(card,clue){
  ...
  const r=await fetch(API,{
    ...
    body:JSON.stringify({card_key:cardKey(card),lesson:i.lesson,vocabulary:i.front,clue:clue})
  });
  let data=null;
...
function showClue(card){
  closeEditor(card);
  let panel=card.querySelector('.vocabulary-clue-panel');
  ...
  panel.querySelector('.clue-edit').addEventListener('click',e=>{
...
function showEditor(card){
  ...
  box.querySelector('.clue-save').onclick=async()=>{
    ...
    const btn=box.querySelector('.clue-save');
    btn.disabled=true;
    try{
      if(v) await save(card,v); else await remove(card);
      box.remove();
      if(v) showClue(card);
    }catch(err){
      console.error('Vocabulary clue save failed:',err);
  ...
  box.querySelector('.clue-delete').onclick=async()=>{
    ...
    try{
      ...
      console.error('Vocabulary clue delete failed:',err);
...
function setup(card){
  ...
  load(card).then(()=>{
    if(card.dataset.vocabularyClue) b.title='Show clue';
  });

  b.addEventListener('click',async e=>{
    e.preventDefault();
    e.stopPropagation();
    if(!card.dataset.vocabularyClue) await load(card);
    showClue(card);
  });
}
...
function styles(){
  ...
  s.textContent=`
.vocabulary-clue-btn{
  grid-column:2;grid-row:1;margin:0!important;padding:0 5px!important;
  border:0!important;background:transparent!important;color:#fff!important;
  font-size:17px!important;line-height:1;cursor:pointer;pointer-events:auto!important;
...
.vocabulary-clue-panel,.vocabulary-clue-editor{
  position:absolute;z-index:100;top:44px;left:8px;right:8px;
  background:#18212c;color:#fff;padding:11px;border-radius:9px;
  box-shadow:0 5px 20px #0009;box-sizing:border-box;
}
.vocabulary-clue-panel{text-align:center;}
.vocabulary-clue-label{font-size:12px;opacity:.75;margin-bottom:5px;}
.vocabulary-clue-text{font-size:15px;line-height:1.45;white-space:pre-wrap;overflow-wrap:anywhere;}
.vocabulary-clue-panel .clue-edit{
  margin-top:9px;padding:5px 14px;border:0;border-radius:6px;cursor:pointer;
}
...
.vocabulary-clue-editor textarea{
  display:block;width:100%;min-height:76px;box-sizing:border-box;resize:vertical;
  padding:8px;border-radius:6px;border:1px solid #66717d;font:inherit;
}
.clue-editor-actions{display:flex;gap:7px;margin-top:7px;}
.clue-editor-actions button{padding:6px 12px;border:0;border-radius:6px;cursor:pointer;}
.clue-editor-actions button:disabled{opacity:.6;cursor:wait;}
@media(max-width:520px){
  .vocabulary-clue-btn{font-size:16px!important;}
  .vocabulary-clue-panel,.vocabulary-clue-editor{top:40px;left:6px;right:6px;}
}
`;
...
function init(){
  ...
  if(grid){
    new MutationObserver(()=>setTimeout(all,0)).observe(grid,{childList:true,subtree:true});
  }
}
