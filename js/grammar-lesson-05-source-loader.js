(function () {
  ...
  function render() {
    ...
    var lesson = lessons[4];
    if (!lesson || !window.grammarLesson05Source) return false;

    var content = lesson.querySelector('.grammar-content');
    if (!content) return false;
    if (content.dataset.lesson05SourceLoaded === 'true') return true;

    content.innerHTML = '';

    window.grammarLesson05Source.forEach(function (rule, index) {
      var item = document.createElement('div');
      item.className = 'gs-source-accordion-item';
      item.id = 'lesson05-source-rule-' + (index + 1);
    ...
    content.dataset.lesson05SourceLoaded = 'true';
    return true;
  }
